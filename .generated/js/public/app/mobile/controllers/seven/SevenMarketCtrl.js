...
(function (intranet) {
    var mobile;
    (function (mobile) {
        class SevenMarketCtrl extends intranet.common.BetControllerBase {
            constructor($scope, $stateParams, $rootScope, $state, $compile, $q, $filter, WSSocketService, marketService, commentaryService, marketOddsService, placeBetDataService, $timeout, localStorageHelper, toasterService, commonDataService, localCacheHelper, modalService, exposureService, promiseTracker, $window, settings, betService) {
                super($scope);
            ...
            getOtherMarkets(eventid, marketid) {
                this.marketOddsService.getOtherMarketsById(eventid, marketid)
                    ...
                    if (response.success) {
                        ...
                        if (this.$scope.otherMarkets.length > 0) {
                            ...
                            var sessionsIndex = intranet.common.helpers.Utility.IndexOfObject(this.$scope.otherMarkets, 'group', intranet.common.enums.MarketGroup.Fancy);
                            if (sessionsIndex > -1) {
                            ...
                            angular.forEach(this.$scope.fancyMarkets, (f) => { super.setSessionPrice(f); });
            ...
            placeBet(m, side, runnerId, price, runnerName, percentage = 100, tdId = 0, trId = 0) {
            ...
            findMarket(marketid) {
                ...
                if (this.$scope.fullMarket.id == marketid) {
                    ...
                    var isFound = false;
                    this.$scope.otherMarkets.forEach((om, index) => {
                        om.markets.forEach((m) => {
                            if (m.id == marketid) {
                                isFound = true;
                                market = m;
                                if (this.$scope.selectedTab != index) {
                                    this.selectedTabChanged(index);
                                }
                            }
                        });
                    });
