...
(function (intranet) {
    var home;
    (function (home) {
        class RadarModalCtrl extends intranet.common.BetControllerBase {
            constructor($scope, $stateParams, marketOddsService, $timeout, toasterService, $rootScope, $state, $compile, $filter, WSSocketService, promiseTracker, marketService, commentaryService, placeBetDataService, localStorageHelper, commonDataService, localCacheHelper, exposureService, settings, betService) {
                super($scope);
            ...
            loadInitialData() {
                ...
                if (result) {
                    this.$scope.stakeConfig = result.stake.filter((a) => { return a.isActive; });
                }
                this.getFullmarket();
            }
            getFullmarket() {
                this.marketOddsService.getFullMarketById(this.$stateParams.marketid)
                    ...
                    if (this.$scope.fullMarket) {
                        if (this.$scope.fullMarket.bettingType == intranet.common.enums.BettingType.ODDS) {
                            this.checkAccess();
                        }
                    }
            ...
            checkAccess() {
                this.$scope.fullMarket.marketRunner.forEach((r) => {
            ...
            cancelAllBets(r) {
                var ids = [];
                var bets = (this.$scope.openBets.filter((ob) => { return ob.runnerId == r.runner.id; }) || []);
                bets.forEach((b) => {
            ...
            prepareView() {
                ...
                this.$scope.fullMarket.marketRunner.forEach((r) => {
                    ...
                    if (tempRadarData.length > 0) {
                        ...
                        var oddsRange = intranet.common.helpers.CommonHelper.getOddsRangeByLadder(min, max, this.$scope.fullMarket.interval, this.$scope.fullMarket.priceLadderType);
                        ...
                        if (r.radarData.length > 0) {
                            ...
                            r.radarData.forEach((rd) => {
                                var findex = intranet.common.helpers.Utility.IndexOfObject(tempRadarData, 'odds', rd.odds);
                                if (findex == -1) {
                                    rd.backSize = 0;
                                    rd.laySize = 0;
                                    rd.layUnmatched = 0;
            ...
            placeBet(side, r, price, percentage = 100) {
                var betmodel = new intranet.common.helpers.BetModal(this.$scope.fullMarket, side, r.runner.id, price, r.runner.name, false, '', percentage);
                var model = betmodel.model;
                if (r.stakeConfig.selectedStake == undefined || r.stakeConfig.selectedStake <= 0) {
                    this.toasterService.showToastMessage(intranet.common.helpers.ToastType.Error, "Please select stake", 1000);
                }
                else {
                    model.size = r.stakeConfig.selectedStake;
                    model.betFrom = intranet.common.enums.BetFrom.Radar;
                            ...
                            if (response.data) {
                                ...
                                if (bet.orderStatus) {
                                    ...
                                    if (bet.sizeRemaining > 0) {
                                        ...
                                        this.toasterService.showToastMessage(intranet.common.helpers.ToastType.Error, unMatchmsg, 1500);
                                    }
                                    else {
                                        var matched = this.$filter('toRate')(bet.sizeMatched);
                                        ...
                                        this.toasterService.showToastMessage(intranet.common.helpers.ToastType.Success, Matchmsg, 1500);
                                    }
                                }
                                else {
                                    var msg = bet.message.format(model.marketName, model.runnerName);
                                    this.toasterService.showToastMessage(intranet.common.helpers.ToastType.Error, msg, 1500);
                                }
                            }
                        }
                        else {
            ...
            selectOneClickStake(runner, stake) {
                runner.stakeConfig.selectedStake = stake.stake;
                runner.stakeConfig.stakeList.forEach((s) => {
                    if (s.id == stake.id) {
                        s.selected = true;
                    }
                    else {
            ...
            oneClickStakeChanged(runner) {
                var selectedstake = runner.stakeConfig.stakeList.filter((s) => { return s.selected == true; });
                if (selectedstake.length > 0) {
                    runner.stakeConfig.selectedStake = selectedstake[0].stake;
                }
                angular.forEach(this.$scope.fullMarket.marketRunner, (m) => {
                    m.stakeConfig.stakeList = runner.stakeConfig.stakeList;
                    if (selectedstake.length > 0) {
                        m.stakeConfig.selectedStake = selectedstake[0].stake;
                    }
                });
            }
            increment(value) {
                var odds = parseFloat(value);
                var interval = 0, frm = 0, to = 0;
                var modulo = 0;
                var range = intranet.common.helpers.Utility.GetPriceLadderUPRange(odds);
                if (range) {
                    frm = range.min;
                    to = range.max;
                ...
                if (runner.priceValue) {
                    ...
                    if (onlyBack || onlyLay) {
                        ...
                        if (backmodel.modulo > 0) {
                            var addition = math.round(math.divide(math.add(backmodel.odds, backmodel.interval), 100), 2);
                            if (1000 >= addition) {
                                price = intranet.common.helpers.Utility.isInt(addition) ? parseInt(addition) : addition;
                            }
                        }
                        if (onlyBack) {
                            betmodel = { backPrice: price };
                        }
                        if (onlyLay) {
                            betmodel = { layPrice: price };
                        }
                    }
                    else {
                        ...
                        if (hasPLUS) {
                            ...
                            var length = 0;
                            if (hasPLUS) {
                                length = (price.match(new RegExp("[+]", "g")) || []).length;
                            }
                            if (length > 1) {
                                betmodel = {
                                    layPrice: length,
                        ...
                        if (betmodel) {
                            ...
                            if (backmodel.modulo > 0) {
                                var addition = math.round(math.divide(math.add(backmodel.odds, backmodel.interval), 100), 2);
                                if (1000 >= addition) {
                                    betmodel.backPrice = intranet.common.helpers.Utility.isInt(addition) ? parseInt(addition) : addition;
                                }
                            }
                            ...
                            for (var i = 0; i < betmodel.layPrice; i++) {
                                ...
                                if (laymodel.interval > 0) {
                                    var addition = math.round(math.divide(math.add(laymodel.odds, laymodel.interval), 100), 2);
                                    if (1000 >= addition) {
                                        layprice = intranet.common.helpers.Utility.isInt(addition) ? parseInt(addition) : addition;
                                    }
                                }
                            }
