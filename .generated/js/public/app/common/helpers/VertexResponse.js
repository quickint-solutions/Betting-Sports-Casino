            ...
            class VertexHelper {
                ...
                validate(response) {
                    ...
                    if (response && jQuery.isArray(response)) {
                        if (response.length == 1) {
                            if (response[0].ID < 0 || response[0].Ticket < 0) {
                                response.splice(0);
                            }
                        }
                        var ve = VertexError.getList();
                        response.forEach((r) => {
                            var result = ve.filter((v) => { return v.Code == r; });
                            if (valid && result.length > 0) {
                                valid = false;
                                this.toasterService.showToast(helpers.ToastType.Error, result[0].Description, 2000);
                            }
                        });
                    }
                    else {
                        ...
                        if (response && helpers.Utility.isInt(response)) {
                            var ve = VertexError.getList();
                            var result = ve.filter((v) => { return v.Code == response; });
                            if (result.length > 0) {
                                valid = false;
                                this.toasterService.showToast(helpers.ToastType.Error, result[0].Description, 2000);
                            }
                ...
                setHistoryType(history) {
                    if (history && jQuery.isArray(history)) {
                        ...
                        history.forEach((h) => {
                            var result = ht.filter((v) => { return v.Code == h.Type; });
                            if (result.length > 0) {
                            ...
                            if (h.Type == 'L') {
                                ...
                                else if (h.ProfitLoss == -3) {
                                    h.pl = 'CANCELLED';
                                }
                                else if (h.ProfitLoss == -4) {
                                    h.pl = 'Hedging';
                                }
                                else {
                                    h.pl = 'Done';
                ...
                groupPosition(response) {
                    var netTrade = [];
                    var groupBy = ((buysell) => {
                        var oneMode = response.filter((b) => { return b.BuySell == buysell; });
                        var symbolId = oneMode.map((b) => { return b.Symbol; });
                        symbolId = symbolId.filter((x, i, a) => a.indexOf(x) == i);
                        symbolId.forEach((b) => {
                            var trade = {};
                            var matched = oneMode.filter((r) => { return r.Symbol == b; });
