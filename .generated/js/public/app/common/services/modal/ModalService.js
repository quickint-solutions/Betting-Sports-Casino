...
(function (intranet) {
    ...
    (function (common) {
        ...
        (function (services) {
            class ModalService {
                constructor($uibModal, settings, localStorageHelper) {
                    this.$uibModal = $uibModal;
                    this.settings = settings;
                    this.localStorageHelper = localStorageHelper;
                    this.options = new services.ModalOptions();
                    this.options.header = 'Proceed?';
                    this.options.body = 'Do you really want to proceed?';
                    this.defaults = new services.ModalDefaults();
                    ...
                    if (!tempDefaults.controller) {
                        tempDefaults.controller = ['$scope', '$uibModalInstance', function ($scope, $uibModalInstance) {
                                ...
                                $scope.modalOptions.ok = result => {
                                    $uibModalInstance.close(services.ModalResult.OK);
                                };
                                $scope.modalOptions.close = result => {
                                    $uibModalInstance.close(services.ModalResult.Cancel);
                                };
                            }];
                    }
                    return this.$uibModal.open(tempDefaults).result;
                }
                showWithOptions(options, defaults) {
                    this.getDefault();
                    var tempDefaults = angular.extend({}, this.defaults, defaults);
                    var tempOptions = angular.extend({}, this.options, options);
                    if (!tempDefaults.controller) {
                        tempDefaults.controller = ['$scope', '$uibModalInstance', ($scope, $uibModalInstance) => {
                                $scope.modalOptions = tempOptions;
                                $scope.modalOptions.ok = result => {
                                    $uibModalInstance.close({ data: tempOptions, button: services.ModalResult.OK });
                                };
                                $scope.modalOptions.close = result => {
                                    $uibModalInstance.close({ data: null, button: services.ModalResult.Cancel });
                                };
                            }];
                    }
                    return this.$uibModal.open(tempDefaults).result;
                }
                showConfirmations(messages, confirmTitle, confirmActionTitle, confirmCancelTitle) {
                    var confirmationMessages = messages
                        ...
                        .map((m) => m.text)
                        .join(' <br /> ');
                    var options = new services.ModalOptions();
                ...
                showConfirmation(message, confirmTitle) {
