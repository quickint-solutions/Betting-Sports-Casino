...
(function (intranet) {
    ...
    (function (common) {
        var directives;
        (function (directives) {
            class KTGridController extends intranet.common.ControllerBase {
                constructor($scope, ngTableParams, $state, modalService, commonDataService, fileService, settings, $window, localStorageHelper, localCacheHelper) {
                    super($scope);
                    this.$state = $state;
                    ...
                    this.sortingCacheKey = 'sorting';
                    this.currentPageCacheKey = 'currentPage';
                    ...
                    $scope.tableParams = new ngTableParams({
                        ...
                        getData: ($defer, params) => {
                            if ($scope.canILoad) {
                                ...
                                var receivedDataFailed = (response) => {
                                    this.$scope.isLoading = false;
                                    if (response.data) {
                                        params.total(0);
                                        $defer.resolve(null);
                                ...
                                if (this.settings.ThemeName == 'lotus')
                                    this.commonDataService.addPromise(promiseItem);
                ...
                primaryDelete() {
                    if (this.$scope.removeItems) {
                        this.$scope.removeItems()
                            .success((response) => this.itemsRemoved(response.data))
                            .error((response) => this.itemsRemovedFailed(response.data));
                    }
                }
                clickExportToExcel() {
                ...
                refreshGridWithId(data) {
                    if (data == this.$scope.gridId) {
                        this.$scope.tableParams.$params.page = 1;
                        this.$scope.tableParams.reload();
                    }
                }
                refreshGridStayOnPage(data) {
                    if (data) {
                        this.$scope.itemToSelect = data.data;
                        this.$scope.message = data.message;
                    }
                ...
                goToDetail(item, dontAutoscroll) {
            ...
            angular.module('kt.components')
                ...
                return {
                    ...
                    templateUrl: settings.ThemeName == 'betfair' ? 'app/common/components/grid/grid-betfair.html' :
                    ...
                    link: (scope, elm, attr, ctrl, transclude) => {
                        ...
                        transclude(scope, (transElem, transScope) => {
                            ...
                            if (attr.dateGroup == 'true') {
                                transScope.dateGroup = true;
                            }
                            else {
                                transScope.dateGroup = false;
                            }
                            if (attr.noPaging == 'true') {
                                transScope.noPaging = true;
                            }
                            else {
                                transScope.noPaging = false;
                            }
                            transScope.isToggleFiltersEnabled = attr.toggleFilters === 'true';
                            if (!angular.isDefined(attr.hasNewRights)) {
                                transScope.hasNewRights = true;
                            }
                            if (!angular.isDefined(attr.hasRemoveRights)) {
                                transScope.hasRemoveRights = true;
                            }
