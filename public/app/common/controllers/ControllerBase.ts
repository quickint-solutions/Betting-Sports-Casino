namespace intranet.common {
    export class ControllerBase<T extends IScopeBase> {
        protected $scope: T;
        ...
        public getUserTypesShort(id: any): any {
            var userTypes: any[] = [];
            userTypes = this.getUserTypes();
            var rr = userTypes.filter((f: any) => { return f.id == id; }) || [];
            if (rr.length > 0) { return rr[0].short; }
            return '';
        }

        public getUserTypesObj(id: any): any {
            var userTypes: any[] = [];
            userTypes = this.getUserTypes();
            var rr = userTypes.filter((f: any) => { return f.id == id; }) || [];
            if (rr.length > 0) { return rr[0]; }
            return '';
        }

        public getUserTypeForPT(id: any): any[] {
            var userTypes: any[] = [];
            angular.forEach(this.getUserTypes(), (u: any) => {
                if (u.id >= id && u.id <= common.enums.UserType.Agent) { userTypes.push(u); }
            });
            return userTypes
        }

        public setOddsInMarket(market: any, r: any, setpl: boolean = true): void {
            if (setpl) market.pl = r.pl;
        ...
        public setSymbolTickData(data: any, lst: any[], selected: any): void {
            if (data && data.data) {
                var d = JSON.parse(data.data);
                var symbol;
                var symbols = lst.filter((s: any) => { return s.code == d.s; }) || [];
                if (symbols.length == 0 && selected.code == d.s) { symbol = selected; }
                if (symbols.length > 0) {
                    symbol = symbols[0];
                    ...
                    if (selected.code == d.s) {
                        if (symbol.symbol == d.s.toUpperCase()) {
                            if (!symbol.tradeList) symbol.tradeList = [];
                            var tt: any = {};
                            tt.price = d.c;
                            tt.size = d.Q;
                            ...
                            if (symbol.tradeList.length >= 20) { symbol.tradeList.pop(); }
                        }
                    }
                }
            }
        ...
        public setSymbolMarkPriceData(data: any, symbol: any): void {
            if (data && data.data) {
                var d = JSON.parse(data.data);
                if (symbol.code == d.s) {
        ...
        public setSymbolDepthData(data: any, symbol: any): void {
            if (data && data.data) {
                var d = JSON.parse(data.data);
        ...
        public setSymbolTradeData(data: any, symbol: any): void {
            if (data && data.data) {
                var d = JSON.parse(data.data);
        ...
        private setOrderBookPrice(priceList: any): void {
            angular.forEach(priceList, (b: any, index: any) => {
                b.price = b[0];
                b.size = b[1];
                b.sum = b.size;
                if (index > 0) {
        ...
        private setPercentage(list: any): void {
