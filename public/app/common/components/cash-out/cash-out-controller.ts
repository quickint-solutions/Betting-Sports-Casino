namespace intranet.common.directives {
    ...
    export interface IKTCashOutScope extends common.IScopeBase {
        market: any;
        openBets: any[];
        timer_calculator: any;
        newBets: any[];
    ...
    class KTCashOutController
        extends common.ControllerBase<IKTCashOutScope> {
        /* @ngInject */
        constructor($scope: IKTCashOutScope,
            ...
            // if (this.settings.IsBetfairLabel == false) {
        ...
        public startProcess(): void {
            ...
            if (this.$scope.market
                ...
                var marketBets = this.commonDataService.openBets.filter((a: any) => { return a.marketId == this.$scope.market.id; });
                if (marketBets.length > 0 && this.$scope.isOpen == true) {
                    this.$scope.newBets = [];
                    var localtooltip = '';
                    this.$scope.market.marketRunner.forEach((m: any) => {
                        m.backTotal = 0; m.layTotal = 0; m.backPL = 0; m.layPL = 0;

                        marketBets[0].bets.forEach((b: any) => {
                    ...
                    if (localtooltip != this.$scope.tooltipHtml) { this.$scope.tooltipHtml = this.$sce.trustAsHtml(localtooltip); }
        ...
        public cashout(): void {
            if (this.$scope.newBets.length > 0) {
                ...
                this.$scope.newBets.forEach((model: any, index: any) => {
