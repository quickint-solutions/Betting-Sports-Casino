module intranet.master {
    export interface IDownlineReportScope extends intranet.common.IScopeBase {
    ...
    export class DownlineReportCtrl extends intranet.common.ControllerBase<IDownlineReportScope>
        implements intranet.common.init.IInit {
        constructor($scope: IDownlineReportScope,
        ...
        private exportToExcel(): void {
            ...
            if (this.$stateParams.memberid) {
                ...
                promise.success((response: common.messaging.IResponse<any>) => {
                        if (response.success) { this.prepareExcel(response.data); }
                    });
            }
            else {
        ...
        private prepareExcel(gridData: any): void {
            if (gridData) {
                ...
                angular.forEach(gridData, (item: any, index: any) => {
                    if (index == 0) {
                     ...
                     angular.forEach(item.userPls, (u: any) => {
                         contentTD = common.helpers.CommonHelper.wrapTD(moment(item.settleTime).format('DD/MM/YYYY'));
                         contentTD += common.helpers.CommonHelper.wrapTD(item.eventTypeName);
                         contentTD += common.helpers.CommonHelper.wrapTD(item.competitionName);

                         if (item.roundId > 0)
                             contentTD += common.helpers.CommonHelper.wrapTD(item.eventName + " # " + item.roundId);
                            else
                             contentTD += common.helpers.CommonHelper.wrapTD(item.eventName);
