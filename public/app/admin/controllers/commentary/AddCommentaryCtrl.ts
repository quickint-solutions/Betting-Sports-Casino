module intranet.admin {
    ...
    export interface IAddCommentaryScope extends intranet.common.IScopeBase {
        ballTypes: any[];
        runTypes: any[];
        wicketTypes: any[];
        otherTypes: any[];
        ...
        ballStatusList: any[];
        scoreProviderList: any[];

        isMute: boolean;
        audioCommentaryMode: boolean;
    }

    export class AddCommentaryCtrl extends intranet.common.ControllerBase<IAddCommentaryScope>
        implements common.init.IInit {
        constructor($scope: IAddCommentaryScope,
        ...
        private fillBallStatus(): void {
            this.$scope.ballStatusList.push({ id: 0, name: 'Ball Complete' });
            this.$scope.ballStatusList.push({ id: 1, name: 'Ball Start' });
        }

        private fillScoreProvider(): void {
            var status: any = common.enums.ScoreSource;
            this.$scope.scoreProviderList = common.helpers.Utility.enumToArray<common.enums.ScoreSource>(status);
        }
        ...
        private loadLastCommentary(isReset: boolean = false): void {
            this.commentaryService.getCommentary(this.$stateParams.eventId)
                .success((response: common.messaging.IResponse<any>) => {
                    if (response.success && response.data) {
                        if (response.data.commentary) { this.$scope.score = JSON.parse(response.data.commentary) };
                        this.$scope.score.scoreSource = response.data.scoreSource;
                        this.$scope.score.ballstatus = 0;
                        if (isReset) { this.$scope.score.commentary = ''; }
                        if (!this.$scope.score.homeTeam) { this.loadTeamNames(); }
                    } else {
                        this.loadTeamNames();
                    }
                });
        ...
        // priority, 1=run,2=ball,3=wicket
        private commentaryClick(item: any, priority: number = 1, considerLastBall: boolean = true): void {
        ...
        private updateCommentary(self: any, item): any {
        ...
        private countOver(self: any, item: any): any {
            if (item.ball > 0) {
                var ball: any = 0;
                var over: any = 0;
                if (self) {
                    ...
                    if (ball == 6) { over = math.add(over, 1); ball = 0; }
                    else {
        ...
        private setLastOver(item: any, priority: number = 1): void {
            if (priority < 4) {
                if (!this.$scope.score.lastOver) { this.$scope.score.lastOver = []; }
                else { if (this.$scope.score.lastOver.length >= 10) { this.$scope.score.lastOver.splice(9); } }
                if (priority == 1) {
        ...
        private removeOverBall(index: any): void {
        ...
        private ballStatusChanged(selectedId: any): void {
            ...
            if (selectedId == 1 && this.$scope.score.scoreSource == common.enums.ScoreSource.Own) {
                this.commentaryClick({ value: 0, ctext: 'ball start' }, 1, false);
            }
        }

        private scoreProviderChanged(selectedId: any): void {
        ...
        private audioConfigChanged(config: any): void {
